...
#
# 3. %$% expositional pipe
# exposes the names of the LHS object to teh RHS expression; 
# helps you avoid using too many $s

# SIMULATION STEPS ----
# 1. Generate Data 
# 2. calculate ols estimates and associated pvales
# 3. RUn x amount of times for given sample size
...
# Directories
dir_data <- "~/projects/are212/section/Section01/"
...
t_stat <- function(data, y_var, X_vars, gamma, intercept = T) {
  ...
  # Force s2 to numeric
  s2 %<>% as.numeric()   
  #Bi-directinal pipe! Feeds input into function, then assigns output value back to input
  # Inverse of X'X
  XX_inv <- solve(t(X) %*% X)
  # Standard error
